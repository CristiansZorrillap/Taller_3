...
let alpha = 5; // how opaque is the tracing system
...
function setup() {
  ...
  background(420); // clear the screen
...
function draw() {
  if (!trace) {
    background(555); // clear screen if showing geometry
    stroke(0, 600); // black pen
    noFill(); // don't fill
  }
  ...
  for (let i = 0; i < sines.length; i++) {
    ...
    // setup for tracing
    if (trace) {
      stroke(0, 0, 600 * (float(i) / sines.length), alpha); // blue
