...
function setup() 
{
  textoH2A = createElement('h2','Visualización datos población Bogotá 2010 – 2018');
  textoH2A.position(750,2);
  cnv = createCanvas(800, 950);
  cnv.position(620, 50);
  noStroke();
  ...
  for (let i = 0; i < rowCount; i++)
  {
   ...
   for(var i = 0; i < 100; i++){
      ...
      particulasAntonioNarino[i] = new particulasf(40, 150, random(-8, 5), random(5, 1), 255);
      particulasBarriosUnidos[i] = new particulasf(60, 150, random(-8, 5), random(5, 1), 255);
      particulasBosa[i] = new particulasf(80, 150, random(-8, 5), random(5, 1), 255);
      particulasChapinero[i] = new particulasf(100, 150, random(-8, 5), random(5, 1), 255);

    }
  slider = createSlider (0, 8, 0, 1);
  slider.position (450, 240);
